...
namespace GazRouter.DAL.Repairs.Plan
{
    ...
    /// Для ГАЗОПРОВОДОВ
    /// </summary>
    public class GetPlanRepairListForPipelinesQuery : QueryReader<GetRepairPlanParameterSet, List<RepairPlanBaseDTO>>
    {
        public GetPlanRepairListForPipelinesQuery(ExecutionContext context)
            : base(context)
        {
        }
...
//                        LEFT JOIN   rd.V_PIPELINE_GROUPS g ON g.pipeline_group_id = rd.P_SEGMENT_BY_GROUP.Get_GROUP_ID(r.Entity_Id, rw.min_kilometer_start)                 LEFT JOIN   rd.V_SITES site on rd.P_ENTITY.GetSiteId(r.Entity_Id, rw.min_kilometer_start) = site.Site_Id   
...
select r.repair_id, r.entity_id, r.entity_name, r.entity_type_id, r.bleed_amount, r.gas_saving,
       r.repair_type_id, r.repair_type_name,
       r.date_start_plan, r.date_end_plan, r.description, r.description_gtp, r.description_cpdd,
       r.date_start_sched, r.date_end_sched,
       r.transfer_fact_w, r.transfer_fact_s, r.transfer_fact_x, r.capacity_plan_w, r.capacity_plan_s, r.capacity_plan_x, r.transfer_work, r.is_transfer_relation, r.execution_means_id, r.is_ext_condition, r.parts_delivery_date, r.complex_id, cmpl.complex_type, cmpl.start_date as complex_start_date, cmpl.end_date as complex_end_date, cmpl.is_local,
       r.upd_entity_name, r.upd_user_name, r.upd_date,
       r.workflow_state, r.repair_state, r.fireworks_type, r.date_start_fact, r.date_end_fact,
       r.resolution_date, r.resolution_num, r.resolution_date_cpdd, r.resolution_num_cpdd,
       r.duration, r.gazprom_plan_id, r.consumers_state, r.plan_reg_date,
       site.site_name, g.pipeline_group_name
  from V_REPAIRS r
  left join V_PPW_COMPLEXES cmpl on r.complex_id = cmpl.complex_id
  left join V_PIPELINES p on p.pipeline_id = r.entity_id
...
AND        
((date_start_plan >= :startdate AND date_start_plan < :enddate) OR (date_end_plan >= :startdate AND date_end_plan < :enddate) OR (:startdate >=date_start_plan AND date_end_plan >= :enddate))
                        AND         r.entity_type_id = 34
                        AND         plan_type_id = 1" +
