...
{
    public class GetSiteToQuery : QueryReader<GetSignersSet, List<TargetingUserDTO>>
    {
        public GetSiteToQuery(ExecutionContext context)
            : base(context)
        { }
...
select 
    ...
    a.agreed_user_id, a.user_id, a.fio_r, a.position_r, a.is_head, 
...
order by l.sortorder";
        ...
        protected override List<TargetingUserDTO> GetResult(OracleDataReader reader, GetSignersSet parameters)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    FIO = reader.GetValue<string>("fio_r"),
                    SortOrder = reader.GetValue<int>("sortorder"),
                    Position = reader.GetValue<string>("position_r"),
