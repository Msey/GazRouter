...
namespace GazRouter.DAL.Repairs.Workflow
{
    ...
    /// ДЛЯ КЦ
    /// </summary>
    public class GetWorkflowRepairListForCompShopsQuery : QueryReader<GetRepairWorkflowsParameterSet, List<RepairPlanBaseDTO>>
    {
        public GetWorkflowRepairListForCompShopsQuery(ExecutionContext context)
            : base(context)
        {
        }
...
FROM
    ...
    LEFT JOIN   V_NM_SHORT_ALL sp ON r.entity_id = sp.entity_id
    LEFT JOIN   V_COMP_SHOPS sh ON sh.comp_shop_id = r.entity_id
    LEFT JOIN   V_PIPELINES p ON p.pipeline_id = sh.pipeline_id
    LEFT JOIN   rd.V_PIPELINE_GROUPS g ON g.pipeline_group_id = rd.P_SEGMENT_BY_GROUP.Get_GROUP_ID(p.pipeline_id, sh.kilometer_conn)                        
            ...
            while (reader.Read())
            {
                ...
                var rep =
                    new RepairPlanCompShopDTO
                    {
                        ...
                        Kilometer = reader.GetValue<double>("kilometer_conn"),
