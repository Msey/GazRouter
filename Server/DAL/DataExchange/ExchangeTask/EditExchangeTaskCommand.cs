...
{
    public class EditExchangeTaskCommand : CommandNonQuery<EditExchangeTaskParameterSet>
    {
        public EditExchangeTaskCommand(ExecutionContext context)
            : base(context)
        {
            IsStoredProcedure = true;
        }

        protected override void BindParameters(OracleCommand command, EditExchangeTaskParameterSet parameters)
        {
            command.AddInputParameter("p_exchange_task_id", parameters.Id);
        ...
        protected override string GetCommandText(EditExchangeTaskParameterSet parameters)
        {
            return "rd.P_EXCHANGE_TASK.Edit";
