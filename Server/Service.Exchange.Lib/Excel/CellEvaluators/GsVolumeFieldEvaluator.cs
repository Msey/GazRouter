...
{
    public class GsVolumeFieldEvaluator : CellEvaluator
    {
        protected override Regex Regex
            => new Regex(@"#FIELD_GS#([^#]*)#([^#]*)#([^#]*)#([^#]*)#([^#]*)#(@TIMESTAMP\s*(([+-])([^+-]+))?)");

        public GsVolumeFieldEvaluator(DateTime timeStamp, PeriodType periodType, ExecutionContext ctx) : base(timeStamp, periodType, ctx)
        {
        }
