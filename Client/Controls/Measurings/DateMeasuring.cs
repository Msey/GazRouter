...
{
    public class DateMeasuring : Measuring<DateTime?>
    {
        public DateMeasuring(Guid entityId, PropertyType propType, PeriodType periodType)
            : base(entityId, propType, periodType)
        {
        }

        public override string DisplayValue => Value?.ToString("dd.MM.yyyy HH:mm");
            ...
            DateTime date)
        {
            ...
            if (valueDict.ContainsKey(EntityId) && valueDict[EntityId].ContainsKey(PropertyType.PropertyType))
            {
                ...
                if (value != null)
                {
                    var dblVal = value as PropertyValueDateDTO;
                    if (dblVal != null)
                    {
                        Value = dblVal.Value;
                    }
