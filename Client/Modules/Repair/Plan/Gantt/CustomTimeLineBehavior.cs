...
namespace GazRouter.Repair.Plan.Gantt
{
    public class CustomTimeLineBehavior: DefaultGanttTimeLineVisualizationBehavior
    {
        protected override IEnumerable<IEventInfo> GetEventInfos(TimeLineVisualizationState state, HierarchicalItem hierarchicalItem)
        {
            ...
            var task = hierarchicalItem.SourceItem as RepairTask;
            if (task != null)
            {
                ...
                if (task.RepairItem.Dto.PartsDeliveryDate.DayOfYear > 1)
                {
                    ...
                    if (range.IntersectsWith(state.VisibleTimeRange))
                        yield return
                            new TimeLineLimiterEventInfo(range, hierarchicalItem.Index,
                ...
                if (task.RepairItem.HasComplex)
                {
                    ...
                    if (startRange.IntersectsWith(state.VisibleTimeRange))
                        yield return
                            new TimeLineLimiterEventInfo(startRange, hierarchicalItem.Index,
                    ...
                    if (endRange.IntersectsWith(state.VisibleTimeRange))
                        yield return
                            new TimeLineLimiterEventInfo(endRange, hierarchicalItem.Index,
                                Color.FromArgb(0xFF, 0xFF, 0x8C, 0x0));


                    // Если если даты проведения работ не соответсвуют датам проведения комплекса
                    if (task.RepairItem.HasComplexError)
                    {
                        var warningRange = new Range<long>(task.RepairItem.Dto.StartDate.Ticks,
                            task.RepairItem.Dto.StartDate.Ticks);
                        if (warningRange.IntersectsWith(state.VisibleTimeRange))
                            yield return
                                new TimeLineWarningEventInfo(warningRange, hierarchicalItem.Index,
                                    task.RepairItem.ComplexErrorString);
                    }
                }
            }
        }
