...
namespace GazRouter.ObjectModel.Model.Dialogs.AddEdit.Pipelines.Chamber
{
    public class AddEditPipelineChamberViewModel : AddEditLinearObjectViewModel<PipelineDTO, Guid>
    {
        private readonly PipelineTypesDTO _newPipelineType;
        private readonly int _sortorder;
        private readonly int _gasTransportSystemId;

        public AddEditPipelineChamberViewModel(Action<Guid> actionBeforeClosing, PipelineDTO model)
            : base(actionBeforeClosing, model)
        {
            ...
            ChamberLgth = (int)(model.Length * 1000);
        ...
        public AddEditPipelineChamberViewModel(Action<Guid> actionBeforeClosing, PipelineType newPipelineType, PipelineDTO parentPipeline, int gasTransportSystemId, int sortorder)
        ...
        public void SetValidationRules()
        {
            ...
            AddValidationFor(() => KilometerOfEndPoint)
                .When(() => KilometerOfStartPoint > KilometerOfEndPoint)
                .Show("Км. начала не может быть больше км. конца.");
            AddValidationFor(() => ChamberLgth).When(() => ChamberLgth < 0).Show("Значение должно быть неотрицательным");
        }
        ...
        private int? _chamberlgth;
        public int? ChamberLgth
        {
            get { return _chamberlgth; }
            set
            {
                if (SetProperty(ref _chamberlgth, value))
                {
                    KilometerOfEndPoint = ChamberLgth / 1000.0;
                    OnPropertyChanged(() => ChamberLgth);
                    SaveCommand.RaiseCanExecuteChanged();
                }
            }
        }
