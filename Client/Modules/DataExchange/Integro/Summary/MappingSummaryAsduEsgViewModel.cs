...
{
    public class MappingSummaryAsduEsgViewModel : LockableViewModel
    {
        ...
        public MappingSummaryAsduEsgViewModel(SummaryItem summary)
        {
        ...
        private void InitCommands()
        {
            ...
            LoadDescriptorCommand = new DelegateCommand(LoadDescriptor, () => SelectedSummary != null && (!MappingDescriptorList.Any()));
        ...
        public SiteDTO SelectedSite
        {
            ...
            set
            {
                ...
                {
                    if (_selectedSite.Name == "Расчётный объект")
                        LoadAggregator();
                    else
                        Refresh();
                }
        ...
        //        {
        //            new SystemItem { Id = (int)MappingSystemType.ASDU_ESG, Name = "АСДУ ЕСГ" },
        ...
        {
            get { return /*SelectedDescriptor != null &&*/ SelectedProperty != null && SelectedMappingParam != null && 
                    (SelectedMappingParam.EntityId == null || Guid.Empty.Equals(SelectedMappingParam.EntityId)); }
        }
        ...
        {
            isMonth = SelectedSummary.Dto.PeriodType == GazRouter.DTO.Dictionaries.PeriodTypes.PeriodType.Month;

            TypeList = (isMonth) ?
                       //new List<string>() { "ГРС","ГИС", "ЛПУ" } :
                       new List<string>() { "ГРС", "ГИС" } :
                       new List<string>() { "КС", "ГРС", "ГИС", "ПРГ", "ЗРА", "Расчётный объект" };
            ...
            if (isMonth)
            {
                ...
                {
                    Id = UserProfile.Current.Site.Id,
                    Name = UserProfile.Current.Site.Name,
                    ParentId = null,
                };
            ...
            {
                var addSite = new SiteDTO();
                addSite.Name = "Расчётный объект";
                SiteList.Add(addSite);
            }
        ...
        private async void Refresh()
        {
            ...
            try
            {
                ...
                if (SelectedSite.Id == UserProfile.Current.Site.Id && isMonth)
                {
                    Items = new List<ItemBase>();
                    var entBinding = new AsduEntityDTO
                    {
                        EntityId = SelectedSite.Id,
                        EntityName = SelectedSite.Name,
                        //EntityGid = _propertyBindings.Where(pb => pb.EntityId == chItem.Id).Select(pb => pb.EntityGid).FirstOrDefault(),
                        EntityTypeId = EntityType.Enterprise,
                    };
                    Items.Add(new BindableItem(entBinding, UpdateEntityBinding));
                    OnPropertyChanged(() => Items);
                    return;
                }
                #region КС
                if (selectedType == "КС"  && !isMonth)
                {
                    // Получить список ГПА
                ...
                if (selectedType == "ГРС")
                {
                    ...
                    foreach (var ds in distrTree.DistrStations)
                    {
                        ...
                        if (SelectedSummary.Dto.PeriodType == GazRouter.DTO.Dictionaries.PeriodTypes.PeriodType.Month)
                        {
                            foreach (var consumer in distrTree.Consumers.Where(o => o.ParentId == ds.Id))
                            {
                                var consBinding = new AsduEntityDTO
                                {
                                    EntityId = consumer.Id,
                                    EntityName = consumer.Name,
                                    //EntityGid = _propertyBindings.Where(pb => pb.EntityId == chItem.Id).Select(pb => pb.EntityGid).FirstOrDefault(),
                                    EntityTypeId = EntityType.Consumer,
                                };
                                var dlItem = new BindableItem(consBinding, UpdateEntityBinding);
                                dlItem.Parent = dsItem;
                                dsItem.Children.Add(dlItem);
                            }
                        }
                        else
                        {
                        ...
                        if (SelectedSummary.Dto.PeriodType == GazRouter.DTO.Dictionaries.PeriodTypes.PeriodType.Month)
                        {
                            var dsBinding =
                                new AsduEntityDTO
                                {
                                    EntityId = ms.Id,
                                    EntityName = ms.Name,
                                    EntityGid =   _propertyBindings.Where(pb => pb.EntityId == ms.Id).Select(pb => pb.EntityGid).FirstOrDefault(),
                                    EntityTypeId = EntityType.MeasStation,
                                    IsActive = entityBindings.Any(id => ms.Id == id)
                                };
                ...
                #endregion
                #region Расчётный объект
                if (selectedType == "Расчётный объект" && !isMonth)
                {
                ...
                #endregion
                #region ЛПУ (Enterprise) 
                //if (selectedType == "ЛПУ" && isMonth)
                //{
                //    LoadEnterprises();
                //}
        ...
        private async void LoadAggregator()
        {
            Lock();
            ...
            foreach (var type in aggrList.GroupBy(a => a.AggregatorTypeName))
            {
                var typeItem = new GroupItem(type.Key) { IsExpanded = false };
                foreach (var chItem in type)
                {
                    var dsBinding = new AsduEntityDTO
                    {
                        EntityId = chItem.Id,
                        EntityName = chItem.Name,
                        //EntityGid = _propertyBindings.Where(pb => pb.EntityId == chItem.Id).Select(pb => pb.EntityGid).FirstOrDefault(),
                        EntityTypeId = EntityType.Aggregator,                        
                    };
                    var dlItem = new BindableItem(dsBinding, UpdateEntityBinding);
                    typeItem.Children.Add(dlItem);
                }
                typeItem.Parent = null;
                Items.Add(typeItem);
                //Items.Add(typeItem);
            }
        ...
        private async void LoadEnterprises()
        {
            Items = new List<ItemBase>();

            if (SelectedSite.EntityType != EntityType.Enterprise)
            {
                OnPropertyChanged(() => Items);
                return;
            }
            ...
            var dlItem = new BindableItem(entBinding, UpdateEntityBinding);
            dlItem.Parent = null;
            Items.Add(dlItem);
        ...
        private async void LoadFromFileSummary()
        {
            ...
            if (openFileDialog.ShowDialog() == true)
            {
                try
                {
                    ...
                    //using (System.IO.FileStream fileStream = openFileDialog.File.OpenRead())
                    //{
        ...
        private void LoadDescriptor()
        {
            ...
            // Пока не грузим описатель
            if (SelectedSystem.Id == -2)
            {
                OpenFileDialog openFileDialog = new OpenFileDialog();
                //openFileDialog.Filter = FILE_FILTER;
                ...
                foreach (var item in res)
                {
                    foreach (var con in item.SummaryParamContentList)
                    {
                        var newItem = new MappingParamDto()
                        {
                            ...
                            ParameterDescriptorNameNew = item.ParameterGid, //при загрузке устаналиваем из базы
        ...
        private async void LinkParam()
        {
            ...
            //SelectedMappingParam.ParameterDescriptorName = (SelectedSystem.Id == (int)MappingSystemType.ASSPOOTI) ? SelectedDescriptor.Key : SelectedDescriptor.Description;
            //
            SelectedMappingParam.ParameterDescriptorName = SelectedMappingParam.ParameterDescriptorNameNew;
            ...
            //if (SelectedSystem.Id == (int)MappingSystemType.ASDU_ESG)
            //{
