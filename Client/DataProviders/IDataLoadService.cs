...
namespace GazRouter.DataProviders.DataLoadMonitoring  
{
    [ServiceContract]
	...
	public interface IDataLoadServiceProxy
	{

        Task<List<SiteDataLoadStatistics>> GetDataLoadSiteStatisticsTechDataAsync(DateTime dt);

        Task<List<BaseEntityProperty>> GetSiteTechDataAsync(EntityPropertyValueParameterSet parameters);

        Task<GasModeChangeData> GetGasModeChangeDataAsync(GasModeChangeParameterSet parameters);

        Task<GasModeChangeData> GetGasModeChangeDataLastSerieAsync();

        Task<GasSupplyDataSetDTO> GetGasSupplyDataSetAsync(int systemId);

        Task<List<GasSupplyValue>> GetGasSupplyValuesAsync(DateTime dt);

        Task<List<GasSupplySumValueDTO>> GetSumGasSupplyValuesByEnterpriseAsync(GasSupplySumParameterSet paramSet);

    }

    public sealed class DataLoadServiceProxy : DataProviderBase<IDataLoadService>, IDataLoadServiceProxy
	{
        protected override string ServiceUri => "/DataLoadMonitoring/DataLoadService.svc";
