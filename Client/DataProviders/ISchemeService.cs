...
namespace GazRouter.DataProviders.ObjectModel  
{
	...
	public interface ISchemeServiceProxy
	{

        Task<int> AddSchemeAsync(SchemeParameterSet parameters);

        Task<List<SchemeVersionItemDTO>> GetSchemeVersionListAsync();

        Task<List<SchemeVersionItemDTO>> GetPublishedSchemeVersionListAsync();

        Task<SchemeVersionDTO> GetSchemeVersionByIdAsync(int parameters);

        Task<int> AddSchemeVersionAsync(SchemeVersionParameterSet parameters);

        Task DeleteSchemeVersionAsync(int parameters);

        Task PublishSchemeVersionAsync(int parameters);

        Task UnPublishSchemeVersionAsync(int parameters);

        Task<SchemeModelDTO> GetFullSchemeModelAsync(int parameters);

        Task CommentSchemeVersionAsync(CommentSchemeVersionParameterSet parameters);

    }

    public sealed class SchemeServiceProxy : DataProviderBase<ISchemeService>, ISchemeServiceProxy
	{
        protected override string ServiceUri => "/ObjectModel/SchemeService.svc";
