...
namespace GazRouter.DataProviders.EventLog  
{
    [ServiceContract]
    ...
    {                   
        ...
        [OperationContract(AsyncPattern = true)]
        [FaultContract(typeof(FaultDetail))]
        IAsyncResult BeginGetQueueExchangeEventList(GetQueueExchangeEventListParameterSet parameters, AsyncCallback callback, object state);
        List<EventExchangeDTO> EndGetQueueExchangeEventList(IAsyncResult result);
	...
	public interface IEventLogServiceProxy
	{

        Task<List<EventDTO>> GetEventListAsync(GetEventListParameterSet parameter);

        Task<List<EventRecepientDTO>> GetEventRecepientListAsync(int parameters);

        Task TakeToControlEventAsync(TakeToControlEventParameterSet parameters);

        Task BackToNormalEventAsync(BackToNormalEventParameterSet parameters);

        Task<Guid> AddEventAttachmentAsync(AddEventAttachmentParameterSet parameters);

        Task EditEventAttachmentAsync(EditEventAttachmentParameterSet parameters);

        Task DeleteEventAttachementAsync(Guid parameters);

        Task<int> AddEventAndRecepientsAsync(AddEventAndRecepientsParameterSet parameters);

        Task EditEventAndRecepientsAsync(EditEventAndRecepientsParameterSet parameters);

        Task EditRecepientsAsync(EditRecepientsParameterSet parameters);

        Task DeleteEventAsync(int parameters);

        Task AckEventAsync(AckEventParameterSet parameters);

        Task<List<EventExchangeDTO>> GetQueueExchangeEventListAsync(GetQueueExchangeEventListParameterSet parameters);

        Task<List<EventTextTemplateDTO>> GetEventTextTemplateListAsync(Guid parameter);

        Task<int> AddEventTextTemplateAsync(AddEventTextTemplateParameterSet parameters);

        Task EditEventTextTemplateAsync(EditEventTextTemplateParameterSet parameters);

        Task DeleteEventTextTemplateAsync(int parameters);

        Task<NonAckEventCountDTO> GetNotAckEventCountAsync(Guid parameters);

        Task<List<EventAnalyticalDTO>> EventAnalyticalListAsync(EventAnalyticalParameterSet parameters);

        Task<List<EventAnalyticalDTO>> EventAnalyticalAckListAsync(EventAnalyticalParameterSet parameters);

        Task MoveToTrashAsync(MoveToTrashEventParameterSet parameters);

        Task RestoreFromTrashAsync(RestoreFromTrashEventParameterSet parameters);

        Task<List<EventAttachmentDTO>> GetEventAttachmentListAsync(int parameters);

    }

    public sealed class EventLogServiceProxy : DataProviderBase<IEventLogService>, IEventLogServiceProxy
	{
        protected override string ServiceUri => "/EventLog/EventLogService.svc";
        ...
        public Task<List<EventExchangeDTO>> GetQueueExchangeEventListAsync(GetQueueExchangeEventListParameterSet parameters)
        {
            var channel = GetChannel();
            return ExecuteAsync<List<EventExchangeDTO>,GetQueueExchangeEventListParameterSet>(channel, channel.BeginGetQueueExchangeEventList, channel.EndGetQueueExchangeEventList, parameters);
        }
