...
namespace GazRouter.DataProviders.Alarms  
{
    [ServiceContract]
	...
	public interface IAlarmsServiceProxy
	{

        Task<List<AlarmDTO>> GetAlarmListAsync(GetAlarmListParameterSet parameter);

        Task<int> AddAlarmAsync(AddAlarmParameterSet parameter);

        Task EditAlarmAsync(EditAlarmParameterSet parameter);

        Task DeleteAlarmAsync(int parameter);

        Task<List<AlarmEventDTO>> GetAlarmEventListAsync(int parameter);

    }

    public sealed class AlarmsServiceProxy : DataProviderBase<IAlarmsService>, IAlarmsServiceProxy
	{
        protected override string ServiceUri => "/Alarms/AlarmsService.svc";
