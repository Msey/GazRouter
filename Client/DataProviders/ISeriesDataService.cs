...
namespace GazRouter.DataProviders.SeriesData  
{
    [ServiceContract]
    ...
    {                   
        ...
        [OperationContract(AsyncPattern = true)]
        [FaultContract(typeof(FaultDetail))]
        IAsyncResult BeginGetSerieCheckList(object parameters, AsyncCallback callback, object state);
        List<SerieCheckDTO> EndGetSerieCheckList(IAsyncResult result);

        [OperationContract(AsyncPattern = true)]
        [FaultContract(typeof(FaultDetail))]
        IAsyncResult BeginUpdateSerieCheck(UpdateSerieCheckParameterSet parameters, AsyncCallback callback, object state);
        void EndUpdateSerieCheck(IAsyncResult result);

        [OperationContract(AsyncPattern = true)]
        [FaultContract(typeof(FaultDetail))]
        IAsyncResult BeginUpdateEntityTypeProperty(UpdateEntityPropertyTypeParameterSet parameters, AsyncCallback callback, object state);
        void EndUpdateEntityTypeProperty(IAsyncResult result);
        ...
        [OperationContract(AsyncPattern = true)]
        [FaultContract(typeof(FaultDetail))]
        IAsyncResult BeginGetGasInPipeChange(DateTime parameters, AsyncCallback callback, object state);
        double EndGetGasInPipeChange(IAsyncResult result);
    }

	public interface ISeriesDataServiceProxy
	{

        Task<List<SeriesDTO>> GetSeriesListAsync(GetSeriesListParameterSet parameters);

        Task<SeriesDTO> GetSeriesAsync(GetSeriesParameterSet parameters);

        Task<SeriesDTO> AddSerieAsync(AddSeriesParameterSet parameters);

        Task<Dictionary<Guid, Dictionary<PropertyType, List<PropertyValueMessageDTO>>>> GetPropertyValueMessageListAsync(GetPropertyValueMessageListParameterSet parameters);

        Task PerformCheckingAsync(List<PerformCheckingParameterSet> parameters);

        Task AcceptMessageAsync(Guid parameters);

        Task<BasePropertyValueDTO> GetPropertyValueAsync(GetPropertyValueParameterSet parameters);

        Task<TrendDTO> GetTrendAsync(GetTrendParameterSet parameters);

        Task<List<BasePropertyValueDTO>> GetPropertyValueListAsync(GetPropertyValueListParameterSet parameters);

        Task<Dictionary<Guid, Dictionary<PropertyType, List<BasePropertyValueDTO>>>> GetEntityPropertyValueListAsync(GetEntityPropertyValueListParameterSet parameters);

        Task SetPropertyValueAsync(List<SetPropertyValueParameterSet> parameters);

        Task<List<SerieCheckDTO>> GetSerieCheckListAsync();

        Task UpdateSerieCheckAsync(UpdateSerieCheckParameterSet parameters);

        Task UpdateEntityTypePropertyAsync(UpdateEntityPropertyTypeParameterSet parameters);

        Task<Dictionary<PropertyType, PropertyValueDoubleDTO>> GetCompUnitPropertyValueListAsync(GetCompUnitPropertyValuesParameterSet parameters);

        Task<CompUnitsOperatingTimeDto> GetOperatingTimeCompUnitListAsync(DateIntervalParameterSet parameters);

        Task<List<GasInPipeDTO>> GetGasInPipeListAsync(GetGasInPipeListParameterSet parameters);

        Task<double> GetGasInPipeChangeAsync(DateTime parameters);

    }

    public sealed class SeriesDataServiceProxy : DataProviderBase<ISeriesDataService>, ISeriesDataServiceProxy
	{
        protected override string ServiceUri => "/SeriesData/SeriesDataService.svc";
        ...
        public Task<List<SerieCheckDTO>> GetSerieCheckListAsync()
        {
            var channel = GetChannel();
            return ExecuteAsync<List<SerieCheckDTO>>(channel, channel.BeginGetSerieCheckList, channel.EndGetSerieCheckList);
        }

        public Task UpdateSerieCheckAsync(UpdateSerieCheckParameterSet parameters)
        {
            var channel = GetChannel();
            return ExecuteAsync(channel, channel.BeginUpdateSerieCheck, channel.EndUpdateSerieCheck, parameters);
        }

        public Task UpdateEntityTypePropertyAsync(UpdateEntityPropertyTypeParameterSet parameters)
        {
            var channel = GetChannel();
            return ExecuteAsync(channel, channel.BeginUpdateEntityTypeProperty, channel.EndUpdateEntityTypeProperty, parameters);
        }
        ...
        public Task<double> GetGasInPipeChangeAsync(DateTime parameters)
        {
            var channel = GetChannel();
            return ExecuteAsync<double,DateTime>(channel, channel.BeginGetGasInPipeChange, channel.EndGetGasInPipeChange, parameters);
        }
